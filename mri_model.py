    ...
    with tf.name_scope(name):
        ...
        if num_features != shape_c:
            shortcut = _conv2d(shortcut, num_features=num_features, kernel_size=1,
                               data_format=data_format, circular=circular)
...
def prior_grad_res_net(curr_x, num_features=32, kernel_size=3,
                       num_blocks=2,
    ...
    with tf.name_scope(name):
        ...
        if do_residual:
            if num_features_in != num_features_out:
                shortcut = _conv2d(net, num_features=num_features_out,
                                   kernel_size=1, data_format=data_format,
                                   circular=circular)
            else:
                shortcut = net
...
def prior_grad_simple(net_input, num_features=128, num_features_out=None, kernel_size=3,
                      num_blocks=5, data_format="channels_last",
                      do_residual=True, circular=True, training=True,
    ...
    with tf.name_scope(name):
        ...
        net_cur = _conv2d(net_cur,
                          num_features=num_features,
                          kernel_size=kernel_size,
                          data_format=data_format,
                          circular=False)
        net_cur = _batch_norm_relu(net_cur,
                                   data_format=data_format,
                                   training=training)
...
def unroll_fista(ks_input, sensemap, num_grad_steps=5,
                 resblock_num_features=128, resblock_num_blocks=2,
                 is_training=True, scope="MRI",
                 mask_output=1, window=None, do_hardproj=True,
                 num_summary_image=0,
    ...
    if verbose:
        print("%s> Building FISTA unrolled network (%d steps)...."
    ...
    with tf.variable_scope(scope):
        ...
        mask=tf.expand_dims(tf.expand_dims(tf.cast(mask, tf.complex64),axis=0),axis=0)
        ks_input = mask * ks_input
        ks_0 = ks_input
        # x0 = A^T W b
        im_0 = tf_util.model_transpose(ks_0, sensemap)
    ...
    if summary_iter is not None:
        tf.summary.image(
            'iter/image', summary_iter, max_outputs=num_summary_image)
    
    return tf_util.complex_to_channels(im_k)
